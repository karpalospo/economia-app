...
import { View, StyleSheet, Image, ScrollView, Text, TextInput, TouchableOpacity, Modal, Alert, AsyncStorage, DeviceEventEmitter, Linking, BackHandler, StatusBar } from "react-native";
...
import { ValidateEmail, CapitalizeWord } from '../../utils/helper';
...
export default class SignUp extends React.Component {
            ...
            if (checkFields.allFieldsAreCorrect === true) {
                ...
                if (!res.error) {
                   ...
                   // await RegisterForPushNotificationsAsync(res.message.data.email)
                    ...
                    this.setState({ loading: false })
                    msg = "No se pudo completar el registro, esto puede ser debido a que el correo electrónico ya existe."
                }
            }
            else {
    ...
    checkEmptyOrInvalidFields = () => {
       ...
       // const gender_error = (this.state.gender == 0);
        ...
        const allFieldsAreCorrect = (!email_error && !password_error && !name_error && !lastname_error && !phone_error && !cellphone_error && !document_error && !address_error);
